...
foreach ($obj in $objwithPerms) {
    ...
    foreach ($permission in $permissions) {
        if ($permissions.count -gt 0) {
            #Do this permissions thing on the other spots too.
...
$z = 1
do {
    $objwithPerms += $AllGroups | Get-Random
    $z++
}while ($z -le $permint)
...
$z = 1
do {
    $objwithPerms += $AllComputers | Get-Random
    $z++
}while ($z -le $permint)
