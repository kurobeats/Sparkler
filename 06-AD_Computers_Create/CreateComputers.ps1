...
Function CreateComputer {
    ...
    if ($WorkstationOrServer -eq 0) {
        if ($WorkstationType -eq 0) {
            #desktop 
            $computernameprefix2 = "WS"
        }
                                        
                                                    
        elseif ($WorkstationType -eq 1) {
            #laptop workflow
            $computernameprefix2 = "LT"
        }
                                                        
        else {
            $computernameprefix2 = "SRV"
        }
    }
    ...
    $results = $null
    try {
        $results = Get-ADComputer $sam -server $setdc
